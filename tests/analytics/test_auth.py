...
def test_the_dashboard_asks_for_hanken_grotesk_and_not_bodoni():
    ...
    for page in ("episode-report", "guest-card", "monthly-review", "numbers-today"):
        ...
        assert request, page
        # The artifacts are where the show's display face belongs: Antonio,
        # as on the public pages since 15 Sep 2026. Bodoni was the first
        # direction's face and must not creep back in.
        assert "Antonio" in request.group(1), page
        assert "Bodoni" not in request.group(1), page
