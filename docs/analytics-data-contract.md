...
> **Filled in on review, 2026-09-16.** The worktree that wrote this had no
> access to the anon key, so `SUPABASE_ANON_KEY` shipped empty with a
> `TODO(nav)`; Nav pasted it from `~/.secrets/secrets.md` (TSN Talks section,
> `anon key`) when the branch was merged. The "not configured" panel the page
> shows when the literal is empty stays, and a test still covers it.
>
> The magic link also needs the project's auth config to know the page: the
> project shipped with `site_url = http://localhost:3000` and an empty redirect
> allow-list, which sends every link to a page that does not exist. Set on
> 2026-09-16 through the management API (`PATCH /v1/projects/{ref}/config/auth`):
> `site_url = https://neramitsingh.github.io/tsntalks/analytics/`,
> `uri_allow_list = https://neramitsingh.github.io/tsntalks/**, http://127.0.0.1:*/**, http://localhost:*/**`.
> When the site moves to Sunny's domain, that is the one setting to change.
...
| `viewsAt(postId, instant)` | `post_snapshots` | `{ views: n\|null, takenAt: Date\|null, stale: boolean }` — the counter at an instant, for the report's "views at 7 days and at 30 days". **null, not 0**, when no snapshot is that old: for every episode published before 14 September 2026 the answer is genuinely unknown, and a zero would read as "nobody watched it in its first week". |
...
`security definer` functions in `004`, which carry an `is_allowed()` guard.

**Checked on review, 2026-09-16: both of those held.** `role_table_grants`
listed `anon` and `authenticated` with every privilege on both views, and
neither view had `security_invoker`, so an anonymous PostgREST call could read
the latest snapshot of every post. `db/005_view_privileges.sql` (applied the
same night) sets `security_invoker = on` on both views, revokes them from
`anon`, `authenticated` and `public`, and revokes `anon` from the schema's
default privileges so the next view is not handed out the same way. The
collector's `live_json()` (service_role) and the `004` functions (run as
`postgres`) are unaffected; `rollup_views`, which is `security invoker`, now
sees exactly what its caller may.

Two more things the review found in `004` before it ran: `demographics_compare`
referenced an alias it never declared (`from cur_rows` where `r.window_start`
needed `from cur_rows r`), which made the whole file roll back on first apply;
fixed in place. And with no user claim on the connection every function returns
zero rows by design, so the smoke was run with `request.jwt.claims` set to the
