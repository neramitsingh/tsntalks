...
language sql stable security definer set search_path = public as $$
  ...
  cur_win  as (select min(r.window_start) as ws, max(r.window_end) as we from cur_rows r),
  prev_win as (select min(r.window_start) as ws, max(r.window_end) as we from prev_rows r)
