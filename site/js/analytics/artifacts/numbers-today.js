...
function resolveDate(value) {
  ...
  const now = new Date();
  if (!value) return now;
  /* The end of the chosen Bangkok day, so "as at 30 September" includes the
     30th rather than stopping at midnight on the 30th. */
  const endOfDay = addPeriods(bangkokDate(value), 'day', 1);
