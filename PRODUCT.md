...
The other two pages live in the same room (16 Sep 2026). The rate card page is the menu in full: it opens on a fork, one episode or a campaign, each quoting its own floor; three frames from the room sit between the fork and the prices; the tiers and the campaigns are rows on the table's wood, one column of prices each, the flagship marked by a rule rather than a different card. The live page keeps its product register but leads with position, the platform's own rolling 90 days in the same strap the home page uses, before the lifetime total; the latest episode is its frame, not its thumbnail, and sits second on the page because that is what Sunny opens it for. The reply channel is one module: with Sunny's LINE link in `site/data/contact.json`, every primary button on every page becomes LINE at once.
